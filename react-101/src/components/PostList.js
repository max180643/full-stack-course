...
const mapPostToComponent = (post) => {
  const { id } = post
  return <Post key={id} post={post} image={post.image} />
  // return <Post key={id} {...post}/>
  // return <Post key={id} title={title} content={content} author={author} /> แบบนี้ส่ง prop ไปทีละตัว
}

const PostList = ({ posts }) => (
  <div>
    <h2 style={{ margin: 10 }}>Post list</h2>
